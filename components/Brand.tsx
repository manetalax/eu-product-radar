...
export default function Brand({ market, inverse = false, asLink = true }: { market?: MarketCode; inverse?: boolean; asLink?: boolean }) {
  const content = <><Mark /><span className="brand-wordmark"><strong>Import</strong> <span>Rules</span> <strong>Verifier</strong></span>{market && <span className="brand-market">{MARKETS[market].code}</span>}</>;

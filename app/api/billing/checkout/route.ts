...
async function reconcileCheckoutSessions(stripe: ReturnType<typeof stripeClient>, customerId: string, userId: string, requestedOption: CheckoutBillingOption) {
  let startingAfter: string | undefined;
  let scanned = 0;
  let generation = 0;
  let reusableUrl: string | null = null;
  do {
    const page = await stripe.checkout.sessions.list({ customer: customerId, limit: 100, ...(startingAfter ? { starting_after: startingAfter } : {}) });
    ...
    for (const session of page.data) {
      const owned = session.metadata?.user_id === userId
        ...
        && isCheckoutBillingOption(session.metadata?.billing_option);
      if (!owned) continue;
      generation += 1;
...
export async function POST(request: Request) {
  ...
  const requestedOption = body?.billingOption
    ...
    ?? 'monthly';
  if (!isCheckoutBillingOption(requestedOption)) return json({ error: b('invalidRequest') }, 400);
  const billingOption: CheckoutBillingOption = requestedOption;
  ...
  try {
    ...
    if (!price.active || price.currency !== 'eur' || price.unit_amount !== expected.amountCents || !recurringMatches) throw new Error(b('stripePrice'));

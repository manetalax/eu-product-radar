...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ message?: string; plan?: string; lang?: string }> }) {
  const { message, plan, lang } = await searchParams;
  ...
  return <LanguageProvider initialLanguage={language}>
    <AuthForm initialMessageKey={isLoginNoticeKey(message) ? message : undefined} requestedPlan={requestedPlan} />
  </LanguageProvider>;
}

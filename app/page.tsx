...
const offerCopy: Record<Language, {
  ...
  es: {
    ...
    monthly: { label: 'Mensual', badge: 'SIN IA', features: ['Análisis de productos ilimitados*', 'PDF y Excel con historial', 'Regulatory Twin + Radar', 'Importaciones y conectores compatibles', 'ImportVerifier AI no incluido'], cta: 'Elegir mensual' },
    annual: { label: 'Anual', badge: 'MEJOR VALOR', features: ['Todo lo del plan Mensual', 'ImportVerifier AI incluido', 'PDF y Excel con historial', 'Regulatory Twin + Radar', 'Conectores de catálogo'], cta: 'Elegir anual' },
    ...
    trialProof: `${FREE_TRIAL_PRODUCT_LIMIT} productos gratis · sin tarjeta`, billingFaqQuestion: '¿Qué cambia entre los planes?', billingFaqAnswer: (monthly, annual, lifetime, custom) => `Mensual (${monthly}) no incluye ImportVerifier AI. Anual (${annual}) sí incluye IA. Lifetime (${lifetime}) mantiene esas funciones con pago único. Personalizada (${custom}) añade dominio, logo, adaptación técnica e integración de WhatsApp.`, fairUse: '*Uso ilimitado sujeto a medidas técnicas razonables contra abuso automatizado.'
  },
  en: {
    ...
    monthly: { label: 'Monthly', badge: 'NO AI', features: ['Unlimited product analysis*', 'PDF and Excel history', 'Regulatory Twin + Radar', 'Compatible imports and connectors', 'ImportVerifier AI not included'], cta: 'Choose monthly' },
    annual: { label: 'Annual', badge: 'BEST VALUE', features: ['Everything in Monthly', 'ImportVerifier AI included', 'PDF and Excel history', 'Regulatory Twin + Radar', 'Catalogue connectors'], cta: 'Choose annual' },
  ...
  fr: {
    ...
    monthly: { label: 'Mensuel', badge: 'SANS IA', features: ['Analyses illimitées*', 'PDF et Excel avec historique', 'Regulatory Twin + Radar', 'Imports et connecteurs compatibles', 'ImportVerifier AI non inclus'], cta: 'Choisir mensuel' },
    annual: { label: 'Annuel', badge: 'MEILLEUR CHOIX', features: ['Tout le Mensuel', 'ImportVerifier AI inclus', 'PDF et Excel', 'Regulatory Twin + Radar', 'Connecteurs catalogue'], cta: 'Choisir annuel' },
  ...
  de: {
    ...
    monthly: { label: 'Monatlich', badge: 'OHNE AI', features: ['Unbegrenzte Produktanalysen*', 'PDF und Excel', 'Regulatory Twin + Radar', 'Kompatible Importe und Connectoren', 'ImportVerifier AI nicht enthalten'], cta: 'Monatlich wählen' },
    annual: { label: 'Jährlich', badge: 'BESTER WERT', features: ['Alles aus Monatlich', 'ImportVerifier AI inklusive', 'PDF und Excel', 'Regulatory Twin + Radar', 'Katalog-Connectoren'], cta: 'Jährlich wählen' },
    ...
    trialProof: `${FREE_TRIAL_PRODUCT_LIMIT} Produkte kostenlos · keine Karte`, billingFaqQuestion: 'Was unterscheidet die Modelle?', billingFaqAnswer: (monthly, annual, lifetime, custom) => `Monatlich (${monthly}) enthält keine ImportVerifier AI. Jährlich (${annual}) enthält AI. Lifetime (${lifetime}) bietet diese Funktionen einmalig. Individuell (${custom}) ergänzt Domain, Logo, technische Anpassung und WhatsApp.`, fairUse: '*Unbegrenzte Nutzung unter angemessenen technischen Schutzmaßnahmen.'
  },
  it: {
    ...
    monthly: { label: 'Mensile', badge: 'SENZA IA', features: ['Analisi prodotti illimitate*', 'PDF ed Excel', 'Regulatory Twin + Radar', 'Import e connettori compatibili', 'ImportVerifier AI non incluso'], cta: 'Scegli mensile' },
    annual: { label: 'Annuale', badge: 'MIGLIOR VALORE', features: ['Tutto il Mensile', 'ImportVerifier AI incluso', 'PDF ed Excel', 'Regulatory Twin + Radar', 'Connettori catalogo'], cta: 'Scegli annuale' },
  ...
  pt: {
    ...
    monthly: { label: 'Mensal', badge: 'SEM IA', features: ['Análises ilimitadas*', 'PDF e Excel', 'Regulatory Twin + Radar', 'Importações e conectores compatíveis', 'ImportVerifier AI não incluído'], cta: 'Escolher mensal' },
    annual: { label: 'Anual', badge: 'MELHOR VALOR', features: ['Tudo do Mensal', 'ImportVerifier AI incluído', 'PDF e Excel', 'Regulatory Twin + Radar', 'Conectores de catálogo'], cta: 'Escolher anual' },
...
export default async function Home({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  const extra = offerCopy[language];
  ...
  return <main className="shell landing-shell">
    ...
    <section className="section methodology-section"><div className="method-copy"><div className="eyebrow">IMPORTVERIFIER INTELLIGENCE</div><h2>AI + Twin + Radar</h2><p className="lead">ImportVerifier separa el análisis determinista de la asistencia de IA para que cada plan sea claro.</p><div className="official-links"><a href={`https://eur-lex.europa.eu/eli/reg/2023/988/oj?locale=${language}`} target="_blank" rel="noopener noreferrer">{t.method.gpsr}</a><a href="https://single-market-economy.ec.europa.eu/single-market/goods/ce-marking/manufacturers_en" target="_blank" rel="noopener noreferrer">{t.method.ce}</a></div></div><div className="method-cards"><article><span>01</span><div><strong>ImportVerifier AI</strong><p>Incluido desde Anual.</p></div></article><article><span>02</span><div><strong>Product Regulatory Twin</strong><p>Estado regulatorio por producto.</p></div></article><article><span>03</span><div><strong>Impact Radar</strong><p>Prioriza cambios y obligaciones.</p></div></article></div></section>
    ...
    <section className="section pricing-section" id="planes">
      ...
      <article className="plan personalized-plan"><span className="plan-label">{extra.custom.badge}</span><b>ImportVerifier · {extra.custom.label}</b><div className="price">{customPrice}</div><span className="plan-cadence">{extra.lifetime.badge.toLocaleLowerCase(localeFor(language))}</span><p className="muted">{extra.custom.body}</p><ul className="plan-features">{extra.custom.features.map(feature => <li key={feature}>✓ {feature}</li>)}</ul><Link className="btn primary plan-button" href={`/login?lang=${language}`}>{extra.custom.cta}</Link></article>

...
test('el borrado de cuenta cancela todas las suscripciones Stripe antes de eliminar Supabase', () => {
  assert.match(accountRoute, /subscriptions\.list/);
  assert.match(accountRoute, /status: 'all'/);
  assert.match(accountRoute, /limit: 100/);
  assert.match(accountRoute, /subscriptions\.cancel/);
  assert.match(accountRoute, /has_more/);
  assert.match(accountRoute, /starting_after/);
  assert.match(accountRoute, /delete-account/);
});
...
test('el webhook solo descarta eventos ya procesados y conserva reintentos para eventos en processing', () => {
  ...
  assert.match(webhook, /received: false, duplicate: true, processing: true \}, \{ status: 409 \}/);

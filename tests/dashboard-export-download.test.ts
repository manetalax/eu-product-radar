...
test('PDF and XLSX exports keep canonical ImportVerifier filenames and delayed object URL cleanup', () => {
  ...
  assert.match(dashboard, /link\.download = `importverifier-\$\{analysisMarket\(current\)\.toLowerCase\(\)\}-\$\{current\.created_at\.slice\(0, 10\)\}-\$\{current\.id\.slice\(0, 8\)\}\.\$\{format\}`/);
  assert.doesNotMatch(dashboard, /import-rules-verifier-/);
  assert.match(dashboard, /document\.body\.appendChild\(link\);[\s\S]*?link\.click\(\);[\s\S]*?link\.remove\(\);/);

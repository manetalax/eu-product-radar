...
- Development/general template may use `free_first` while testing fallback behavior.
...
- Images use free-first/free-only vision routing.
...
- PDF/DOC/DOCX/ODT still need a local/free parser to preserve full universal-input coverage under `free_only`; until then they must fail clearly rather than incur a paid fallback.
- OpenAI remains optional fallback only when policy permits it.
...
- Product extraction routes image + text-like files through free AI first; RTF locally converted to text.
...
- Radar ingest environment variable: `REGULATORY_INGEST_SECRET`.
...
- Global security headers: nosniff, frame denial, strict referrer policy, restrictive permissions policy; API/auth no-store.
- Release checker requires only active Stripe Unlimited env, accepts free AI without requiring OpenAI, validates free_only/SiliconFlow pairing, and validates strong Radar ingest secret when configured.
...
3. Connect first verified official EU feed to Radar ingestion without inventing or scraping undocumented APIs.
4. Continue mobile/iPad/PWA release QA and camera/file-flow hardening.
...
3. Add first official-source Radar adapter only after verifying documented source/export behavior.
...
6. Update this file and long handoff.
